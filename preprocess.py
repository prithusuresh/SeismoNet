...
#         print (x.shape)
        ...
        groundTruth2 = []
#         print ("Generating...")
        for resp,scg,ecg1,rpeak1,ecg2,rpeak2 in generator:
            if ecg1.shape[0] != wlen*fs or ecg2.shape[0] != wlen*fs or scg.shape[0] != wlen*fs or resp.shape[0] != wlen*fs or rpeak1 is None or rpeak2 is None:
                continue
            ...
            respSig.append(resp.reshape((1,-1)))
        ...
        scgSig_t = torch.tensor(scgSig).type(torch.float)
        respSig_t = torch.tensor(respSig).type(torch.float)
        ecg1Sig_t = torch.tensor(ecg1Sig).type(torch.float)
        ecg2Sig_t = torch.tensor(ecg2Sig).type(torch.float)

        inputSig_t = torch.cat((scgSig_t, respSig_t),1)
...
#         print("{} inputsig: {} gt1: {} gt2: {} bothecg: {}".format(p_no,inputSig_t.shape,groundTruth1_t.shape,groundTruth2_t.shape,ecg12Sig_t.shape))
